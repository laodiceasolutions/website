...
export default async function Page({ params }) {
  ...
  const dictionary = await getDictionary(lang);
  const article = dictionary.blog.WCDFY[slug];
  return <><JsonLd data={blogPostingJsonLd({ locale: lang, title: article.title, description: article.summary, ...articleConfig })} /><WebAndMobileClient /></>;
}
